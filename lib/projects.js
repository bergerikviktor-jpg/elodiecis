...
/**
 * Lägger till en update på client-docens activeProjectsCount i en batch.
 * Ett projekt anses "aktivt" så länge status === "active". Cache:n
 * underhålls i createProject / archiveProject / unarchiveProject /
 * deleteProjectAndChildren / updateProject (vid clientId-byte).
 */
function _bumpClientProjectsCount(batch, teamId, clientId, delta) {
  if (!clientId || delta === 0) return;
  batch.update(doc(db, TEAMS, teamId, CLIENTS, clientId), {
    activeProjectsCount: increment(delta),
...
export async function createProject(
  ...
  const ref = doc(collection(db, TEAMS, teamId, PROJECTS));
  ...
  // Nya projekt börjar alltid som "active" → bumpa kundens räknare.
  _bumpClientProjectsCount(batch, teamId, clientId, +1);
  await batch.commit();
  return ref.id;
}

export async function updateProject(teamId, projectId, updates) {
  // Om clientId ändras måste vi flytta cache-räknarna mellan
  // gammal och ny kund. För alla andra fält räcker ett vanligt
  // updateDoc — billigare än att alltid läsa docen först.
  const touchesClient = "clientId" in updates;
  const ref = doc(db, TEAMS, teamId, PROJECTS, projectId);
  ...
  const newClientId = updates.clientId || null;
  const isActive = before.status === "active";

  const batch = writeBatch(db);
  batch.update(ref, { ...updates, updatedAt: serverTimestamp() });
  if (oldClientId !== newClientId && isActive) {
    _bumpClientProjectsCount(batch, teamId, oldClientId, -1);
    _bumpClientProjectsCount(batch, teamId, newClientId, +1);
  }
  await batch.commit();
}
...
export async function archiveProject(teamId, projectId) {
  ...
  // Läs först för att hinka kund-cachen.
  const snap = await getDoc(ref);
...
export async function unarchiveProject(teamId, projectId) {
  ...
  const data = snap.exists() ? snap.data() : null;
  const clientId = data?.clientId || null;
  const wasArchived = data?.status === "archived";
 ...
 *
 * Vid radering läser vi först projekt-docen för att kunna dekrementera
 * kundens cache om projektet var aktivt och hade en koppling.
 */
export async function deleteProjectAndChildren(teamId, projectId) {
  ...
  // Radera projektet + dekrementera kund-cache i samma batch.
  const finalBatch = writeBatch(db);
  finalBatch.delete(projectRef);
  if (data?.clientId && data?.status === "active") {
    _bumpClientProjectsCount(finalBatch, teamId, data.clientId, -1);
  }
