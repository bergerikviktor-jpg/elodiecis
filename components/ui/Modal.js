...
export default function Modal({ isOpen, onClose, title, subtitle, children, size = "md", className }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className={cn(
        "relative w-full rounded-2xl bg-white border border-slate-200 shadow-hover animate-slide-in-up",
        ...
        {title && (
          <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
            ...
            <button onClick={onClose} className="p-2 rounded-lg text-slate-400 hover:text-[#0052FF] hover:bg-blue-50 transition-all duration-200">
              <X className="w-4 h-4" />
            </button>
          </div>
        )}
